...
async function updateInfo() {
  ...
  console.log('currentUniqueDonors', currentUniqueDonors);

  const { raisedCentsNew, uniqueDonorsNew } = valueDataVal;
  const raisedDollars = raisedCentsNew / 100;
  ...
  if (uniqueDonorsNew > currentUniqueDonors) {
    donorDifference = uniqueDonorsNew - currentUniqueDonors;
    console.log('donorDifference', donorDifference);
...
/**
  This code is only formatted as such to make sure it works in Codepen. For Streamlabs, use the following:
	...
	document.addEventListener('goalLoad', function(obj) {
		...
		$('.goal-cont__progress').css({width: (percentageAchieved > 100) ? 100  + "%" : percentageAchieved + "%"});
	});
